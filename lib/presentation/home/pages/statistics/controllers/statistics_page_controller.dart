...
class StatisticsPageController extends GetxController {
  ...
  fetchDefaultStatistics() {
    getStatistics(
        position: "FORWARD", clubId: 1); // Default clubId and position
  }

  callPLayerDetailPage(PlayerDetail player, context,StatisticsPageController controller,int index ) {
    Navigator.of(context).push(MaterialPageRoute(builder: (BuildContext ctx) {
      return PlayerDetailPage(player: player, controller: controller, index: index,);
    }));
  }
}
